...
static DS18B20_Command matchRom = {.code = 0x55, .rxBytesNum = 0, .txBytesNum = 8};
...
/*----------------------------------------------------------------------------*/
DS18B20_Status DS18B20_MatchRom(DS18B20 *sensor)
{
  DS18B20_Status result;
  uint8_t id[8];
  id[0] = FAMILY_CONSTANT;
  for (uint8_t i = 0; i < 7; i++){
	  id[i+1] = sensor->serialNumber[i];
  }
  id[7] = CalculateChecksum(id, 7);
  result = ExecuteCommand(sensor, matchRom, id);
...
/*----------------------------------------------------------------------------*/
void DS18B20_ReadTemp(DS18B20 *sensor) {
	DS18B20_InitializationCommand(sensor);
	DS18B20_MatchRom(sensor);
	DS18B20_ConvertT(sensor, 0x01);
...
/*----------------------------------------------------------------------------*/
void DS18B20_GetSerial(DS18B20 *sensor, UART_HandleTypeDef *huart) {
	sensor->isConnected = 0;
	sensor->uart = huart;
	sensor->isInitialized = 1;
	DS18B20_InitializationCommand(sensor);
	uint8_t rxData[DS18B20_READ_ROM_RX_BYTES_NUM];
	ExecuteCommand(sensor, readRom, rxData);
	for (uint8_t i = 0; i < DS18B20_SERIAL_NUMBER_LEN_BYTES; i++)
	{
	sensor->serialNumber[i] = rxData[DS18B20_SERIAL_NUMBER_OFFSET_BYTES + i];
	}
